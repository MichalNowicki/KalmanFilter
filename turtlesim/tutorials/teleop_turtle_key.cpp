...
private:
  ...
  bool stopRequested;
  ros::Publisher twist_pub_, accel_pub_;
  ...
  a_scale_(0.5)
{
  ...
  accel_pub_ = nh_.advertise<geometry_msgs::Accel>("turtle1/cmd_acc", 1);
...
//    printf("value: 0x%02X\n", c);
    ...
    switch(c)
    {
      ...
      accel.angular.z = a_scale_*angular_;
      accel.linear.x = l_scale_*linear_;
      if(twistSend == true || stopRequested == true)
      {
          if (stopRequested)
              accel.linear.z = 1;
