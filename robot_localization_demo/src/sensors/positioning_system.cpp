...
namespace robot_localization_demo {
  ...
  TurtlePositioningSystem::TurtlePositioningSystem(ros::NodeHandle node_handle, std::string topic_out, double frequency,
      double error_x_systematic, double error_x_random, double error_y_systematic, double error_y_random,
      double error_yaw_systematic, double error_yaw_random, int turtle_color_red, int turtle_color_green, int turtle_color_blue,
      bool visualize):
    node_handle_{node_handle},
    turtle_pose_subscriber_{node_handle_.subscribe("turtle1/pose", 16, &TurtlePositioningSystem::turtlePoseCallback, this)},
    turtle_pose_publisher_{node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>(topic_out, 16)},
    ...
    frame_sequence_{0},
    turtle_color_red_{turtle_color_red},
    turtle_color_green_{turtle_color_green},
    turtle_color_blue_{turtle_color_blue},
  ...
  void TurtlePositioningSystem::spawnAndConfigureVisualizationTurtle(const turtlesim::Pose & initial_pose) {
    if(isVisualizationRequested() && !isVisualizationTurtleAvailable()) {
      ...
      configure_visualization_turtle.request.r = turtle_color_red_;
      configure_visualization_turtle.request.g = turtle_color_green_;
      configure_visualization_turtle.request.b = turtle_color_blue_;
      ...
      // Log message.
      ROS_INFO("Absolute position measurement visualized by '%s' using a (%d,%d,%d) pen.", visualization_turtle_name_.c_str(),
               turtle_color_red_, turtle_color_green_,turtle_color_blue_);
    }
  }
